...
- Added an attainment history for each quarter, with a chart on the report showing converted ACR climbing toward your target week by week. It follows the workload dropdown, so you can see progress for just the workloads you own instead of a territory-wide total. MSXi publishes a new snapshot weekly and only keeps about seven weeks of them, so Sales Buddy stores each week's totals permanently - your quarter's progress stays visible long after MSXi drops it. The line can dip, because MSXi sometimes restates conversions downward.
...
- Turn milestone agenda items into linked engagements, capture discussion notes against both records, and edit engagement story details without leaving the 1:1 workspace.
