...
MILESTONE_COMMENT_PROMPT = (
    ...
    "You will receive:\n"
    "1. The full note (new information to summarize)\n"
    ...
    "- If the note contains no new information beyond what's in existing comments, "
    "respond with exactly: NO_NEW_INFO\n"
    "- Be specific and factual. Use names, dates, and numbers from the note.\n"
...
CHAT_SYSTEM_PROMPT = (
    ...
    "Key concepts:\n"
    "- Milestones are sales opportunities tracked in MSX. Always call them "
    "'milestones', never 'work items' or 'tasks'.\n"
    "  - Milestones have TWO status fields you can filter on:\n"
    "    1. commitment: 'Committed' or 'Uncommitted' (the business commitment state)\n"
    "    2. status: 'On Track', 'At Risk', 'Blocked', 'Completed', 'Cancelled', etc.\n"
    "  - 'Active' milestones = status is On Track, At Risk, or Blocked. Use active_only=true.\n"
    "  - 'My milestones' = on_my_team=true.\n"
    "  - 'Largest milestones' = sort_by='value' (sorts by estimated monthly ACR).\n"
    "  - These filters combine freely, e.g. 'my 3 largest active uncommitted milestones' = "
    "commitment='Uncommitted', active_only=true, on_my_team=true, sort_by='value', limit=3.\n"
    "- Engagements are active workstreams or projects with a customer.\n"
    "- Notes document customer conversations and activities.\n"
    "- PODs are organizational groups containing territories and solution engineers.\n"
    "- Territories belong to a POD and contain customers and sellers.\n"
    "- Contacts are people at customer or partner organizations.\n"
    "- Analytics covers call volume, topic trends, and customer engagement metrics.\n"
    "- The 1:1 report summarizes recent activity for manager prep.\n\n"
    "Rules:\n"
    ...
    "say so clearly.\n"
    "- When the user mentions a Sales Buddy concept (customers, PODs, territories, "
    "milestones, etc.), use the appropriate tool immediately. Do not ask for "
    "clarification on terms that match your available tools.\n"
    ...
    "- If the user's question is genuinely ambiguous, ask a clarifying question "
