...
**EVERYTHING goes in the partial.** That means:
- The HTML layout
...
**For modal rendering:** When loading a partial via `fetch()` + `innerHTML`, `<script>` tags won't auto-execute. Add a small script activator loop in the modal JS to clone and re-insert script elements. That is the ONLY new code needed - everything else comes from the partial.

**For DOMContentLoaded:** Scripts in the partial that use `document.addEventListener('DOMContentLoaded', ...)` won't fire when injected into an already-loaded page. Change these to immediate invocations (the script runs at injection time anyway, so this is equivalent).
